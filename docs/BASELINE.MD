...
### Run-time profiling
The previous version took ~20 minutes on 20k simulations using parallelisation with 10 parallel processes. To get a better estimate I also run this without parallelisation but using 2k simulations which takes ~5 minutes (very slow).
...
Note that for each iteration of $n_{sim}$ we loop over all values of $m$ (call them $m_{i}$) and for each of those we run all the scenarios. If we denote as $n_{scenario}$ the number of scenarios and $n_{metrics}$ the number of metrics, the computational complexity is for a single $m_i$ is:

$O\left(n_{scenarios} \cdot (m_i\log m_i+n_{metrics}))\right)$
