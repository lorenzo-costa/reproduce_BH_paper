...
if __name__ == "__main__":
    try:
        sim_out_new = pd.read_csv(
            "data/benchmark_new/simulated/full_simulation_results.csv"
        )
        sim_out_new_parallel = pd.read_csv(
            "data/benchmark_new_parallel/simulated/full_simulation_results.csv"
        )
        sim_out_old = pd.read_csv(
            "data/benchmark_old/simulated/full_simulation_results.csv"
        )
        sim_out_old_concat = pd.read_csv(
            "data/benchmark_old_concat/simulated/full_simulation_results.csv"
        )
    ...
    agg_new = (
        sim_out_new.groupby(["method", "m", "m0_fraction", "scheme", "L"])
        .mean()
        .reset_index()
    )
    agg_new_parallel = (
        sim_out_new_parallel.groupby(["method", "m", "m0_fraction", "scheme", "L"])
        .mean()
        .reset_index()
    )
    agg_old = (
        sim_out_old.groupby(["method", "m", "m0_fraction", "scheme", "L"])
        .mean()
        .reset_index()
    )
    agg_old_concat = (
        sim_out_old_concat.groupby(["method", "m", "m0_fraction", "scheme", "L"])
        .mean()
        .reset_index()
    )
    ...
    for metric in metrics:
        ...
        if not np.allclose(agg_new[metric], agg_old[metric], atol=tolerance):
            print(
                f"Discrepancy of {np.max(np.abs(agg_new[metric] - agg_old[metric]))} found in metric {metric} between new and old simulation!"
            )
            all_good = False
        if not np.allclose(agg_new_parallel[metric], agg_old[metric], atol=tolerance):
            print(
                f"Discrepancy of {np.max(np.abs(agg_new_parallel[metric] - agg_old[metric]))} found in metric {metric} between new parallel and old simulation!"
            )
            all_good = False
        if not np.allclose(agg_old_concat[metric], agg_old[metric], atol=tolerance):
            print(
                f"Discrepancy of {np.max(np.abs(agg_old_concat[metric] - agg_old[metric]))} found in metric {metric} between old concatenated and old simulation!"
            )
            all_good = False
    if all_good:
